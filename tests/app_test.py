...
def search(client, query):
    return client.get("/search", query_string=dict(query=query), follow_redirects=True)
...
def test_search(client):
    ...
    def assert_successful_search(html_content, allowed_titles=None, allowed_texts=None):
        ...
        for title, text in zip(test_titles, test_texts):
            if title in allowed_titles or text in allowed_texts:
                assert (
                    bytes(title, "utf-8") in html_content
                    and bytes(text, "utf-8") in html_content
                )
            else:
                assert (
                    bytes(title, "utf-8") not in html_content
                    and bytes(text, "utf-8") not in html_content
                )

    # Test empty query
    assert_successful_search(
        search(client, query="").data, allowed_titles=[], allowed_texts=[]
    )

    # Test that all title and text entries match
    assert_successful_search(
        search(client, query="title").data,
        allowed_titles=test_titles,
        allowed_texts=test_texts,
    )
    assert_successful_search(
        search(client, query="text").data,
        allowed_titles=test_titles,
        allowed_texts=test_texts,
    )
    ...
    assert_successful_search(
        search(client, query=test_titles[0]).data,
        allowed_titles=test_titles[:1],
        allowed_texts=test_texts[:1],
    )
    assert_successful_search(
        search(client, query=test_texts[0]).data,
        allowed_titles=test_titles[:1],
        allowed_texts=test_texts[:1],
    )
